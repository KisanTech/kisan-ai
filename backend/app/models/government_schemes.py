...
class SchemeQueryRequest(BaseModel):
    ...
    language: Optional[str] = Field("english", description="Response language (hindi, english, kannada)")
...
class FarmerProfile(BaseModel):
    ...
    land_size: Optional[str] = Field(None, description="Size of farmer's land (e.g., '2 acres')")
    crop_type: Optional[str] = Field(None, description="Type of crop grown")
    location: Optional[str] = Field(None, description="Farmer's location (state/district)")
    farmer_category: Optional[str] = Field(None, description="Farmer category (small/marginal/medium/large)")
...
class SchemeInfo(BaseModel):
    ...
    description: Optional[str] = Field(None, description="Brief description of the scheme")
    eligibility: Optional[List[str]] = Field(None, description="Eligibility criteria")
    benefits: Optional[List[str]] = Field(None, description="Key benefits")
    required_documents: Optional[List[str]] = Field(None, description="Required documents")
    application_process: Optional[List[str]] = Field(None, description="Application steps")
    portal_url: Optional[str] = Field(None, description="Official portal URL")
...
class DocumentValidationResult(BaseModel):
    ...
    recommendations: Optional[List[str]] = Field(None, description="Recommendations for missing documents")
...
class CorpusStatus(BaseModel):
    ...
    corpus_name: Optional[str] = Field(None, description="Corpus name")
    display_name: Optional[str] = Field(None, description="Display name")
    file_count: int = Field(0, description="Number of files in corpus")
    embedding_model: Optional[str] = Field(None, description="Embedding model used")
    chunk_size: Optional[int] = Field(None, description="Chunk size for documents")
    chunk_overlap: Optional[int] = Field(None, description="Chunk overlap")
    top_k: Optional[int] = Field(None, description="Top K retrieval setting")
...
class SchemeQueryResponse(BaseModel):
    ...
    retrieved_context: Optional[str] = Field(None, description="Retrieved context from RAG")
    error: Optional[str] = Field(None, description="Error message if any")
...
class CorpusRefreshResponse(BaseModel):
    ...
    corpus_status: Optional[CorpusStatus] = Field(None, description="Updated corpus status")
    timestamp: datetime = Field(default_factory=datetime.now, description="Response timestamp")
